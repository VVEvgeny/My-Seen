    ...
    public class BrowserActionFilterAttribute : ActionFilterAttribute, IActionFilter
    {
        void IActionFilter.OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (browserName != BrowserTypes.Chrome.ToString().ToLowerInvariant()
                ...
                &&
                ((browserName != BrowserTypes.Ie.ToString().ToLowerInvariant() &&
                  browserName != BrowserTypes.InternetExplorer.ToString().ToLowerInvariant()) || majorVersion < 9))
                filterContext.Result =
                    new RedirectToRouteResult(new RouteValueDictionary(new {controller = "BrowserFilter"}));
