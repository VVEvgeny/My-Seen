...
function showRoad(roadInfo) {
    ...
    arrayOfStrings.forEach(function(item) {
        if (item) {
            trackCoordsLatLng.push(new window.google.maps
                .LatLng(parseFloat(item.split(",")[0].trim()), parseFloat(item.split(",")[1].trim())));
        }
    });

    addPolyline(trackCoordsLatLng);

    addMarker(trackCoordsLatLng[0],
        roadInfo.Name + " - " + roadInfo.DateText + " (" + roadInfo.DistanceText + ")",
        "start");
    addMarker(trackCoordsLatLng[trackCoordsLatLng.length - 1],
        roadInfo.Name + " - " + roadInfo.DateText + " (" + roadInfo.DistanceText + ")",
        "end");
};