...
namespace MySeenWeb.Models
{
    public class HomeViewModelSerials
    {
        ...
        public HomeViewModelSerials(string userId, int page, int countInPage, string search, string shareKey, ICacheService cache)
        {
            Pages = cache.Get<Pagination>(cache.GetFormatedName(CacheNames.UserSerialsPages.ToString(), userId, page, countInPage, search, shareKey));
            Data = cache.Get<IEnumerable<SerialsView>>(cache.GetFormatedName(CacheNames.UserSerials.ToString(), userId, page, countInPage, search, shareKey));
            ...
            if (Pages == null || Data == null)
            {
                ...
                if (Pages == null)
                {
                    ...
                    cache.Set(cache.GetFormatedName(CacheNames.UserSerialsPages.ToString(), userId, page, countInPage, search, shareKey), Pages, 15);
                }
                if (Data == null)
                {
                    ...
                    cache.Set(cache.GetFormatedName(CacheNames.UserSerials.ToString(), userId, page, countInPage, search, shareKey), Data, 15);
                }
            }
